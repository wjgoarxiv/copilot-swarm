...
> [!NOTE]
> CSW는 Copilot의 네이티브 병렬 하위 작업을 "완료"를 단언이 아니라 **증거 기반 oracle**로
> 판정하는 계획 → 실행 → 리뷰 루프로 감쌉니다. Copilot 스케줄러를 대체하지 않으며 네이티브
> 확장 표면(플러그인 매니페스트·스킬·커스텀 에이전트·훅·네이티브 task/fleet 스케줄링)만으로
> 구현되며 **런타임 의존성 0개**입니다.
