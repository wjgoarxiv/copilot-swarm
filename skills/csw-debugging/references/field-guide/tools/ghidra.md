# Ghidra for authorized binary diagnosis

Use only on binaries the user owns or is authorized to inspect. Installation is a separate state
change and is not authorized by this reference. Prefer an existing verified installation and keep
projects in an exact session-owned path with an explicit retention decision.
