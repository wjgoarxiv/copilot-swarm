...
| **MCP server** | Invoke the tool via its actual client (provider model Desktop, Cursor, etc. if available) or `mcp-cli`, not just the HTTP probe endpoint. The MCP handshake itself is sometimes where bugs live. |
| **Native binary** | Re-run the exact command that crashed or misbehaved. If the input was a file, use the same fixture. Confirm the reproduction with the repository's existing harness or an approved minimal driver. Capture exit code, signal if any, and core dump if generated. |
...
- [runtimes/bundled-app-binary.md](../runtimes/bundled-app-binary.md) — `process.env.X` baked at build time, dead code from tree-shaking failures, worker sub-bundles diverging from main bundle
