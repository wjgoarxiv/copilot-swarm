# Bundled application binaries

Use this reference only for an artifact the user owns or is authorized to inspect. Extracted
source, strings, maps, credentials, and proprietary assets are sensitive temporary material: keep
them outside version control, minimize access, record exact paths, and remove only those paths
after verification. Prefer official source maps or build artifacts over heuristic extraction.
...
```
  ...
  [4] Source-level static analysis (rg plus focused file inspection)
...
rg -n "provider-model-opus-4-7|reasoning_effort|api_key" extracted-bundle.js
...
## Cleanup for bundled-application work

Record every extracted file and directory at creation time. Before cleanup, confirm the path is
inside the approved session root, is not a symlink escape, and is not an input supplied by the
user. Remove exact paths without globs, verify their absence, and retain an artifact only when the
user or audit contract explicitly requires it. Never commit extracted proprietary source, maps,
credentials, or string dumps.
