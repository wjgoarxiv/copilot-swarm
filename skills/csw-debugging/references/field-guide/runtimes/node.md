...
```bash
deno run --inspect-brk --allow-read=src --allow-env src/main.ts
...
| HTTP 200 + response in <1s for an LLM call | Too fast for a real provider model/GPT call; something short-circuited |
...
# Reverse only the instrumentation hunk after inspecting the current diff; preserve user changes.
