...
When starting a new TypeScript project (or scripting against the world), the choice of runtime, package manager, framework, and toolchain compounds. The wrong default at minute zero costs hours every week. The right defaults for current:
...
Use Node when:
- A dependency uses native modules Bun can't load (rare in current; check the dep's release notes)
...
`bun x` replaces `npx`. `bun create` scaffolds projects.

## Package manager — pnpm > npm

Use the package manager selected by the repository lockfile and contribution instructions. For a
new project, choose deliberately and document the choice; do not migrate an existing repository
merely because another manager is preferred.
...
- You have an ESLint plugin Biome doesn't replicate (rare in current)
...
`verbatimModuleSyntax: true` enforces explicit `import type { ... }` for type-only imports — pairs with the optional smell rule on type-only imports.
