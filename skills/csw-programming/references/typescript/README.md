...
> **HTTP client gate** - preserve the client already selected by the repository. Whichever API is
> used, make timeout, abort, retry, redirect, response validation, and cleanup policy explicit.
> Introduce `ky`, `undici`, or another dependency only for a measured capability gap and verify
> the current package and security status before changing the manifest.
| Testing | `bun test` / vitest | Fast, ESM-native |
| Logging | `pino` | Structured JSON, fast |
| CLI | `@clack/prompts` + `commander` | Interactive + parsing |

## tsconfig — a strict configuration example

For an explicitly requested new project, start with the repository's chosen runtime and package
manager, then use `tsconfig-strict.md` as a reviewable menu. This package does not bundle or run a
project generator and does not authorize installing a different toolchain.
...
Use the repository's configured compiler and lint rules. The table below is a review checklist,
not a claim that a bundled scanner exists.
