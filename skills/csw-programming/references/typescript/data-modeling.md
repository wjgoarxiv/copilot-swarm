...
Do not add a runtime schema library to internal-only data by default. Use it when the internal
boundary still receives untrusted values or the repository has standardized on shared schemas;
otherwise prefer the existing type and invariant pattern.
