...
This is the reference Release/Acquire pattern. **Use `OnceLock<Config>` instead** in new code — it encapsulates exactly this with safe API.

## Std vs parking_lot vs tokio for locks

| | std::sync::Mutex | parking_lot::Mutex | tokio::sync::Mutex |
|---|---|---|---|
| Speed | Slowest (OS futex direct) | Often performant (smarter parking) | Slow (await-aware) |
