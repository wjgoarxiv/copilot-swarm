# PydanticAI Reference (v1.x, current)

> Reference patterns for wiring PydanticAI agents. Target: production usage, late-2025 / current.
...
| `anthropic:` | `'anthropic:provider-model-sonnet-4-6'`, `'anthropic:provider-model-opus-4-1'` |
| `google-gla:` | `'google-gla:gemini-3-flash-preview'` |
| `google-vertex:` | `'google-vertex:gemini-3-pro-preview'` |
| `bedrock:` | `'bedrock:anthropic.provider-model-sonnet-4-6'` |
...
**Note**: `result.data` was renamed; the reference accessor is `result.output`.
...
- **V1** reached API stability in September 2025. Breaking changes are reserved for V2 (earliest April current).
- **v1.88.0** renamed `result_type` → `output_type` and `result_tool_name` / `result_tool_description` were removed. Use `output_type`.
- The reference accessor for run results is `result.output` (not `result.data`).
