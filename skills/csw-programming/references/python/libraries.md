# Python library decision tree

Treat this file as a capability index. The active environment, `pyproject.toml`, lockfile,
repository instructions, and CI are authoritative. Do not replace an existing library because a
different example appears here. For a new dependency, verify its current official documentation,
maintenance and security posture, supported Python versions, license, and packed behavior.
...
## HTTP client — [httpx](https://github.com/pydantic/httpx)
...
**Install**: `httpx[http2]` — always include all three extras, no exceptions.

A directly constructed `httpx.AsyncClient()` or `httpx.Client()` is acceptable when its ownership,
timeouts, transport, and closure are explicit. For shared production clients, use the lifecycle
decision process in `http-client-lifecycle.md`; enable only options supported by the service and
measured workload:
...
_LIMITS = httpx.Limits(max_connections=200, max_keepalive_connections=40, keepalive_expiry=30.0)
...
transport = httpx.AsyncHTTPTransport(http2=True, retries=3, limits=_LIMITS, socket_options=_SOCKET_OPTS)
async with httpx.AsyncClient(transport=transport, timeout=_TIMEOUT, follow_redirects=True) as client:
    response = await client.get("https://api.example.com/users")
    response.raise_for_status()
    users = response.json()

# Sync
transport = httpx.HTTPTransport(http2=True, retries=3, limits=_LIMITS, socket_options=_SOCKET_OPTS)
...
See `references/http-client-lifecycle.md` for the full factory functions (`create_client()` / `create_async_client()`), event hooks, and the rationale behind every setting. **Load that reference whenever you write ANY network code.**
...
**Critical current fact**: with Pydantic v2, `model.model_dump_json()` is backed by pydantic-core (Rust) and is faster than `orjson + default=` bridge for Pydantic-shaped responses. **Use `model_dump_json()` for Pydantic; orjson for everything else.**
...
asyncpg is the often performant Python Postgres driver, native to SQLAlchemy 2.x async, native to FastAPI's lifespan model. URL: `postgresql+asyncpg://user:pass@host:5432/db`.
