...
The reference web API stack. Async end-to-end, type-safe end-to-end, OpenAPI-generated end-to-end.
...
Use the serializer already selected by the service. Consider `orjson` only when compatibility and a
representative measurement justify it; configure response behavior deliberately and test typed,
raw, streaming, error, and non-JSON paths. See `orjson-stack.md` for decision factors rather than a
universal default.
...
Wait — that comment violates the optional smell rule. Use proper field defaults instead. Real version:
