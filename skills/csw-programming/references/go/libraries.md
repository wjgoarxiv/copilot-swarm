# Go library decision tree

This is a capability map, not a default dependency set. Start from `go.mod`, imports, generated
code, repository instructions, and CI. Keep the standard library or existing framework when it
satisfies the contract. A dependency change needs a concrete capability gap, maintenance and
security review, compatibility proof, and an approved version.

---

## HTTP framework — preserve the repository choice

The examples use `gin`, `chi`, and `net/http` to show different tradeoffs. They do not assert
market share or authorize migration. Select among them only for a new service or an explicit
migration after checking middleware needs, standard-handler interoperability, generated APIs,
observability, team familiarity, and the repository's current dependency policy.
...
Other frameworks may be correct when already established. Evaluate their current maintenance,
security posture, standard-library compatibility, and migration cost instead of banning them by
name.

See `backend-stack.md` for the gin reference layout, middleware ordering, SSE, graceful shutdown, structured logging integration.

---

## RPC — protocol and compatibility gate

The example below demonstrates Connect. Preserve raw gRPC, Connect, or another established
protocol unless the requested behavior requires a migration and all clients can be proven
compatible.
...
go install github.com/sqlc-dev/sqlc/cmd/sqlc@<pinned-version>
...
If the repository already uses `gorm`, `ent`, or another data layer, preserve that architecture
unless migration is explicitly in scope. For new work, compare query visibility, generation,
reflection, graph modeling, migration ownership, and team familiarity against the actual service.
...
```bash
go install github.com/spf13/cobra-cli@<pinned-version>
...
Default: `net/http.Client` with a tuned `http.Transport`. The stdlib client is **already excellent** in current — HTTP/2 by default, connection pooling, sane timeouts when configured.
...
Use an existing HTTP client consistently. For a new dependency decision, compare header and retry
visibility, middleware, streaming, cancellation, testability, and dependency cost; the standard
library is often sufficient, but repository requirements decide.
...
Stdlib `crypto/*` for everything. For password hashing, **argon2id is the current standard** — bcrypt is acceptable but argon2 is OWASP's recommendation since 2023.
...
See `testing.md` for reference patterns.
...
- bubbletea v2 IME reference for the `SetVirtualCursor(false)` pattern
