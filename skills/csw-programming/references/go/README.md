...
Production Go in current. **Boring on purpose, strict by tooling, illegal states unrepresentable by convention.**
...
Read these per-file references for the reference patterns:
...
- **HTTP backend** → `backend-stack.md` — `gin` server skeleton, middleware ordering, SSE/streaming with `http.Flusher`, structured slog logging, graceful shutdown — distilled from the CLIProxyAPI codebase (a real proxy serving OpenAI/Gemini/provider model APIs).
...
- **Bootstrap** → `bootstrap.md` — reviewable project layout (`cmd/`, `internal/`, `pkg/`), task runner, and CI choices.
- **Strict config** → `golangci-strict.md` — the reference `.golangci.yml` with the full linter whitelist and per-linter rationale.
