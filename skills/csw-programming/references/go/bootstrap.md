...
Use this project-layout reference only when the user explicitly asks for a new Go project or an
approved structural migration. It explains reviewable choices; this skill does not bundle a
generator or automatically create the tree.
...
## Optional tooling (approval required)

The following commands are examples for a newly approved toolchain. Do not execute them in an
existing repository unless its manifest or explicit user direction selects those tools. Replace
the version placeholder with a reviewed, pinned version.

These are CLI tools, installed once per machine via `go install`:

```bash
go install mvdan.cc/gofumpt@<pinned-version>
go install golang.org/x/tools/cmd/goimports@<pinned-version>
go install github.com/golangci/golangci-lint/cmd/golangci-lint@v2.0.0
go install go.uber.org/nilaway/cmd/nilaway@<pinned-version>
...
go install go.uber.org/mock/mockgen@<pinned-version>
go install github.com/go-task/task/v3/cmd/task@<pinned-version>
```

For Connect/protobuf projects, additionally:

```bash
go install github.com/bufbuild/buf/cmd/buf@<pinned-version>
go install google.golang.org/protobuf/cmd/protoc-gen-go@<pinned-version>
go install connectrpc.com/connect/cmd/protoc-gen-connect-go@<pinned-version>
```

## Project layout — the reference tree
...
If the project adopts this template, create the files through the normal reviewed edit workflow
and verify them in a disposable directory before merging them into an existing tree.
