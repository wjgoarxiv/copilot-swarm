...
The reference Go CLI skeleton. `cobra` is the de facto framework — Kubernetes, Docker CLI, Helm, GitHub CLI, gh, Hugo all use it. Use it.
...
`SetArgs` + `ExecuteContext` is the reference pattern. Bind a ctx with a short deadline for tests that would otherwise block.
